...
We used a set-up described for example in \cite{backoff}. However, since we did not have Russian morhpology fully working, we used only the system described as \texttt{lemma backoff} -- with the exception of not translating to lemma. We were not using interpolated backoff, simply because regular backoff is easier to use with Moses.
