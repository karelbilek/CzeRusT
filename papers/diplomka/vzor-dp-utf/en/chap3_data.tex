...
In this thesis I will call the corpus \uv{Mixed Intercorp}.

\subsubsection{Original Intercorp}

Because we wanted some additional data for testing purposes, we decided to take more data from Intercorp. We were given access to \uv{raw} Intercorp data for non-commercial, academic purposes.
...
To have a separate set, we removed the data from the \uv{mixed} Intercorp.
...
\subsubsection{Czech-Russian subtitels}
...
We decided to use UMC 0.1 corpus as \uv{umc-train}, and from 0.3 we use the test part as \uv{umc-test} and the devel part as \uv{umc-devel}.
...
The corpus is categorized by year, and we treat each year as its own corpus for the interpolation (as described in TODO).%\ref{interpol}).
...
The version of Yandex corpus that we used was originally lowercased. Since then, Yandex already made a new version with the correct cases; we did not use the new version for any experiments for time contrains.
...
\jednatabulkan{statrumono} { |r|r |r | r |r|r| }
{
         \hline
         \textbf{Corpus} &
\textbf{Lines}
