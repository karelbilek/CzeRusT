...
Every TectoMT task can be decomposed into so-called \emph{blocks}. To quote \cite{zabokrtsky}: \uv{The basic processing units are blocks. They serve for some very limited, well defined, and often linguistically interpretable tasks (e.g., tokenization, tagging, parsing).} In other words, block is given a tree as an input and the block then does some well-defined task on it.
...
However, the transfer and Russian synthesis doesn't seem to be very advanced. Transfer of \uv{t-lemmas} actually only involves look-up in a very small dictionary, made by taking the proprietary PC-Translator dictionary (see section TODO) and taking a subset of its lexical lemmas that also appear in UMC (see TODO). Transfer of formemes is just a few hand-written rules.
